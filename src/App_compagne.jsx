...
// CONSTANTS & SEED DATA
// ─────────────────────────────────────────────────────────────────────────────
const DEFAULT_SETTINGS = { goalKcal: 1500, goalWeight: 60, startWeight: 82, intolerances: ["lactose", "sucre"] };
const MACROS = { protein: 120, carbs: 150, fat: 50 };
const TABS = [
...
export default function App() {
  ...
  useEffect(() => {
    (() => {
      const savedDays = sGet("nt_comp_days") || {};
      const savedWeights = sGet("nt_comp_weights") || [];
      const savedSettings = sGet("nt_comp_settings");
      const savedFavs = sGet("nt_comp_favorites") || [];
      const savedCustomDb = sGet("nt_comp_customdb") || [];
  ...
  function saveToCustomDb() {
    ...
    setCustomDb(next); sSet("nt_comp_customdb", next);
    alert(`✅ "${cName}" ajouté à ta base perso !`);
  }
  function removeCustomDb(id) { const next = customDb.filter(f=>f.id!==id); setCustomDb(next); sSet("nt_comp_customdb", next); }
  ...
  function saveFavorite() {
    ...
    const next = [...favorites, fav]; setFavorites(next); sSet("nt_comp_favorites", next);
  }
  function removeFav(id) { const next = favorites.filter(f=>f.id!==id); setFavorites(next); sSet("nt_comp_favorites", next); }
  ...
  function addWeight() {
    ...
    setWeightLog(next); sSet("nt_comp_weights", next); setNewWeight("");
  }

  function saveSettings() {
    const next = { ...settings, goalKcal: +editGoalKcal||1900, goalWeight: +editGoalWeight||105 };
    setSettings(next); sSet("nt_comp_settings", next);
  ...
  function importData(e) {
    ...
    r.onload = async (ev) => {
      try {
        ...
        if (data.days) { setAllDays(data.days); sSet("nt_comp_days", data.days); }
        if (data.weights) { setWeightLog(data.weights); sSet("nt_comp_weights", data.weights); }
        if (data.settings) { setSettings(data.settings); sSet("nt_comp_settings", data.settings); }
        if (data.favorites) { setFavorites(data.favorites); sSet("nt_comp_favorites", data.favorites); }
        if (data.customDb) { setCustomDb(data.customDb); sSet("nt_comp_customdb", data.customDb); }
        alert("Import réussi ✅");
...
- Activité : sédentaire | Intolérances : lactose et sucre ajouté (JAMAIS proposer)
